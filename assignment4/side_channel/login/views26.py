...
def safe_check(username: str, password: str) -> bool:
    """
    Safe password check that avoids prefix-based timing leakage.
    """
    # Compare both values in constant time.
    # This prevents early exit based on matching prefix characters.
    username_ok = hmac.compare_digest(username, SECRET_USERNAME)
    password_ok = hmac.compare_digest(password, SECRET_PASSWORD)
